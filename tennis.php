...
<script>
        ...
        fetch('get_score.php?game=tennis')
            ...
            .then(data => {
                if(data.success) {
                    ...
                    // Tennis Score (Points & Sets)
                    const p1 = scores.current_game_t1 || '0';
                    const p2 = scores.current_game_t2 || '0';
                    ...
                    // Timeout (Generic support if admin adds it later)
                    ...
                    if(scores.is_timeout) {
                        ...
                        document.getElementById('to-t1-score').innerText = scores.sets_won_t1 || 0; // Show Sets in Timeout? Or Points? Sets is safer for global context
                        document.getElementById('to-t2-score').innerText = scores.sets_won_t2 || 0;
                    } else {
    ...
    function triggerVisualAnimation(text, teamName, duration = 4000) {
        ...
        content.className = "text-center position-relative p-5 type-winner"; // Default to winner style for Tennis context mostly
        
        txt.innerText = teamName;
        tm.innerText = text; // Subtext (WINNER)
