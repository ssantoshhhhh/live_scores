...
<div class="container py-5 scoreboard-container theme-volleyball" id="app">
    <div class="text-center mb-4">
        <span class="badge bg-secondary mb-2">LIVE VOLLEYBALL MATCH</span>
    ...
    <div class="row align-items-center justify-content-center">
        ...
        <div class="col-md-5 mb-4">
            <div class="glass-card p-5 text-center position-relative">
                <i class="fas fa-volleyball-ball text-danger position-absolute top-0 start-50 translate-middle d-none" id="serve-t1"></i>
        ...
        <div class="col-md-5 mb-4">
            <div class="glass-card p-5 text-center position-relative">
                <i class="fas fa-volleyball-ball text-danger position-absolute top-0 start-50 translate-middle d-none" id="serve-t2"></i>
    ...
    // Upcoming
    $upcoming = $conn->query("SELECT * FROM matches WHERE game_type='volleyball' AND status='scheduled' ORDER BY start_time ASC LIMIT 5")->fetchAll(PDO::FETCH_ASSOC);
    
    // Pagination Logic for Previous
    $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
    $limit = 5;
    $offset = ($page - 1) * $limit;
    $total_stmt = $conn->query("SELECT COUNT(*) FROM matches WHERE game_type='volleyball' AND status='completed'");
    $total_matches = $total_stmt->fetchColumn();
    $total_pages = ceil($total_matches / $limit);
    
    $prev_sql = "SELECT * FROM matches WHERE game_type='volleyball' AND status='completed' ORDER BY start_time DESC LIMIT :limit OFFSET :offset";
...
<script>
    function updateScore() {
        fetch('get_score.php?game=volleyball')
            .then(r => r.json())
            .then(data => {
                if(data.success) {
                    ...
                    document.getElementById('team1-score').innerText = scores.current_points_t1 || '0';
                    document.getElementById('team2-score').innerText = scores.current_points_t2 || '0';
